window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		const tagId$1 = "@haibala/dsh-remote-web-ui/remote.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId$1) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "@haibala/dsh-remote-web-ui";
		...
		*/
		...
		const tagId = "@haibala/dsh-remote-web-ui/settings-card.module.css";
		...
		*/
		...
		const zh = {
			...
			"settings.notifyOnCompleteHint": "智能体结束一轮任务时，电脑和已打开的手机 /m 播放提示音，并在浏览器授权后弹出系统通知。手机需保持该页面开启；通知正文只含会话标题。",
			"settings.inherit": "继承",
			"settings.on": "开",
			"settings.off": "关",
			...
			"update.missingDetail": "无法定位已安装的 Haibala 聚合包。请先运行 dsh plugin --profile <profile> add @haibala/dsh-remote-web-ui。",
		...
		const en = {
			...
			"settings.notifyOnCompleteHint": "When an agent goes idle after running, the desktop and an open phone /m page play a chime and show a system notification (after the browser grants permission). Keep /m open on the phone. The notification body is the session title only.",
			...
			"update.missingDetail": "Could not locate the installed Haibala package. Install it with: dsh plugin --profile <profile> add @haibala/dsh-remote-web-ui",
		...
		*/
		...
		async function runAccept(token, page) {
			...
			if (ok) if (isMobileSurface()) page.navigate("/m");
			else page.reload();
		}
		/**
