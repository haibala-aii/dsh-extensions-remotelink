...
function makeMobileRoutes() {
	...
	const handleBundle = async (_req, res) => {
		...
		if (!existsSync(path)) {
			writeStatic(res, 503, "text/plain", "mobile bundle not built: run pnpm --filter @haibala-aii/dsh-extensions-remotelink build");
			return;
		}
		try {
...
//#endregion
//#region ../../packages/host/apiproxy/src/api/rpc.ts
/**
...
function makeMobileApiRoutes(deps) {
	...
	const handleMethod = async (req, res) => {
		...
		try {
			writeJson(res, 200, await dispatch(apiProxy, method, parsed?.payload, rpcId));
		} catch (error) {
			writeJson(res, 200, {
	...
	const handleHost = async (req, res) => {
		if (req.method !== "GET") {
			res.writeHead(405);
			res.end();
			return;
		...
		req.on("close", onClose);
		try {
			const frames = apiProxy.events.host({
				rpcId: RpcId(`mobile-host-${Date.now().toString(36)}`),
				payload: {}
			}, controller.signal);
			for await (const frame of frames) {
				if (closed) break;
	...
	return [
		...
		{
			kind: "exact",
			path: MOBILE_API_PATHS.host,
			handler: handleHost
		},
		{
			kind: "exact",
			path: MOBILE_API_PATHS.prefs,
			handler: handlePrefs
		}
	];
}
...
const AGGREGATE_PACKAGE = "@haibala-aii/dsh-web-ui-all";
/** Fallback anchor: this plugin's own package when the aggregate is absent. */
const SELF_PACKAGE = "@haibala-aii/dsh-extensions-remotelink";
...
function familyChildren(anchorManifest) {
	...
	const names = [];
	for (const [name, spec] of Object.entries(dependencies)) if (name.startsWith("@haibala-aii/") && typeof spec === "string") names.push(name);
	return names;
}
/**
