...
Haibala 的 DeepSeek Harness Web 移动端远程控制：电脑生成一次性二维码，手机扫码后直接使用正常响应式 Web 界面。
...
2. 设置 → 远程：打开插件并保存。
3. 手机扫码（或打开复制的链接），配对成功后自动刷新进入正常 Web 界面（小屏下侧边栏/详情栏会自动收起）。
...
智能体结束一轮任务后，电脑端和已打开的手机页面会播放提示音，并在浏览器授权后弹出系统通知。设置 → 远程 → **任务完成提醒** 同时控制两端。手机请保持页面开启；Android Chrome 在后台标签页仍可弹出通知。通知正文只有会话标题，不含网址或路径。
