...
export interface RemotePanelProps {
  ...
  state: PanelState
  copied: boolean
  /** Inline settings-page layout: no dialog chrome or close control. */
  embedded?: boolean
  onClose(): void
  onStop(): void
  onRefresh(): void
 ...
 */
export function RemotePanel({ t, state, copied, embedded, onClose, onStop, onRefresh, onCopy, onPickAddress, onPickPublic }: RemotePanelProps) {
  return (
    <div
      className={embedded === true ? css.panelEmbedded : css.panel}
      role={embedded === true ? 'region' : 'dialog'}
      aria-modal={embedded === true ? undefined : 'true'}
      aria-label={t('title')}
    >
